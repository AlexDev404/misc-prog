...
function webhost_setup() {
	...
	add_image_size( 'webhost-large-thumb', 830 );
	add_image_size( 'webhost-medium-thumb', 550, 400, true );
	add_image_size( 'webhost-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'webhost' )
	) );
...
add_action('webhost_before_site', 'webhost_preloader');
...
function webhost_grid_post_meta(){
...
function webhost_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'webhost') . '</a>';
}
add_filter('excerpt_more', 'webhost_excerpt_more');
...
function webhost_scripts() {
	...
	wp_enqueue_script( 'webhost-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'webhost-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'webhost-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
 ...
 * Enqueue Bootstrap
 */
function webhost_enqueue_bootstrap() {
	wp_enqueue_style( 'webhost-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'webhost_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'webhost_widgets_init' );
function webhost_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'webhost' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'webhost' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'webhost' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'webhost' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function webhost_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'webhost' ), get_the_title() ),
...
function webhost_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'webhost').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "webhost");
        echo '"<em>';
...
function webhost_recommended_plugin() {
...
add_action( 'tgmpa_register', 'webhost_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'webhost_check_for_update' );
function webhost_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 30,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/WebHost_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/WebHost',
	'twitter' => 'https://twitter.com',
	'facebook' => 'https://www.facebook.com/',
	'website' => 'http://suavethemes.com',
]);

function webhost_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'webhost_upsell_script' );